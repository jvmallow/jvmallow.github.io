...
Here's the problem: the structure of the hydrogen gas can be obtained by exact calcuation. The spectrum of helium and all the other elements can only be obtaind approximately. This has nothing to do with quantum physics. It turns out, and was proved mathematically centuries ago, that the two-body problem is the only one that can be solved exactly. And hydrogen gas is the only case of this. So physicists have constructed elaborate schemes to get good approximate results for the others; i.e., those that match experiment--in our case, spectra.  The one I worked on is called the Self Consistent Field Theory. It sound worse than it is. Imagine an atom with a few electrons, and one in a larger orbit than all the rest. What does it see? It sees the nucleus surrounded by a sort of cloud of "electromagnetic field" of the other others. For example, sodium is one such. It has a nucleus of charge +11 and eleven electrons, each of charge -1. The eleventh one is at a considerably larger orbit than the othrers. (Chemists call this the valence electron.) In other words, it sees a cloud of approximate charge +1. But that's like hydrogen. So it can be solved exactly! Well, not so fast. That cloud isn't really a cloud, it's ten points zipping along in their orbits around the nucleus. Counting the nucleus and the valence electron, all in all, that's a 12-body problem! So now the self-consistency kicks in. The problem as, we said, can only be solved numerically. So we start with the valence-electron cloud model and do a hydrogen-like calculation. But we have to do it for all the electrons. The lucky valence electron sees an approximately spherical cloud. Each of the others, however, is embedded in a cloud that surrounds it. The computer then calculates the approximate orbit of each of the electrons in turn. It does it again and again, until the latest calculation gives the same answer as the one before it: that's what is meant by self-consistency. A good way to think of it is that each electron contributes to the field just what it sees from the field. All of these calculations are not actually dealing with electrons as particles. They are solving the Schroedinger equation again and again and again.
