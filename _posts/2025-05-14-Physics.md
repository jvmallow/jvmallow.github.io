...
I was part of a group that studied fog by lidar, using a pulsed ruby laser as the transmitter and the telescope at Northwestern University's Lindheimer Astrophysical Research Center (LARC) as the detector. We found that the backscattered signal from the fog was stable over a period of hours, which we had not expected. I also undertook to analyze the "distribution function" of fog droplets; i.e., how many of each size there might be.  I chose reasonable guesses for the smallest and the largest droplets.  FYI,their radii are on the order of micrometers: millionths of a meter. Droplets of that size scatter light, making it hard to drive in fog. Not so for rain, whose droplets re much larger, and less susceptible to light scattering.
